...
def book(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.error(
                request,
                "Welcome to Little Lemon. Your table has been booked.",
...
#     if request.method == "POST":
#         # date = datetime.today().date()
#         # booking_date = request.POST.get("reservation_date")
...
# def edit_booking(request, bookingId):
#     try:
#         booking = get_object_or_404(Booking, bookingId=bookingId, user=request.user)
#         if booking == None:
#             messages.error(
#                 request,
...
#             return redirect(
#                 "bookings"
#             )  # Redirect to the reservations page after editing
#     else:
#         form = BookingForm(instance=booking)
